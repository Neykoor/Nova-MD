...
export async function startBot() {
  ...
  if (loginMethod === 'code' && !sock.authState.creds.registered) {
    ...
    setTimeout(() => requestPairingCodeWhenReady(sock, phone), 3000)
  }
  ...
  sock.ev.on('group-participants.update', async (event) => {
    try {
      ...
      for (const participant of event.participants) {
        const participantJid = typeof participant === 'string' ? participant : participant?.id || participant?.jid || participant?.lid
...
async function handleMessage(sock, m) {
  ...
  const chatConf = db.getChat(m.chat)
  const activePrefixes = chatConf.prefix ? [chatConf.prefix] : config.prefix
  const prefixUsed = activePrefixes.find((p) => m.body.startsWith(p))
