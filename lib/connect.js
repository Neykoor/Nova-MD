...
async function requestPairingCodeWhenReady(sock, phone, attempt = 1) {
  ...
  let waited = 0
  while (sock.ws?.readyState !== 1 && waited < maxWaitMs) {
    await delay(waitStepMs)
    waited += waitStepMs
  }

  if (sock.ws?.readyState !== 1) {
    console.log(chalk.red('[NOVA MD] El socket no llegó a abrirse a tiempo, no se puede pedir el código.'))
    return
  }
...
export async function startBot() {
  ...
  const sock = makeWASocket({
    ...
    browser: loginMethod === 'code' ? Browsers.macOS('Desktop') : [config.botName, 'Chrome', '1.0.0'],
